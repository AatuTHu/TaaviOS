...
Since all kernel tasks share the same address space, userspace requests can be dropped directly into task queues via syscalls. So there is no expensive context switches or message-passing overhead. Each task enforces its own memory boundaries through the **Ledger Protocol**, actively validating every allocation before operating on it. This is not hardware-enforced isolation it is a deliberate tradeoff: less overhead, higher trust requirements per task.
...
Each kernel task that manages domain memory registers with the **Ledger Protocol** at init time. From that point, all dynamic allocations go through the ledger rather than directly to the kernel heap.

The ledger assigns each task a private virtual floor in a reserved region of the address space (`0xD0000000` and upward). When a task allocates memory, the ledger records the real heap address internally and hands back a *protocol address* a synthetic address within that task's floor. The real address is never exposed outside the ledger. When any code wants to operate on a ledger-managed allocation, it must present its pid and the protocol address. The ledger validates ownership and returns the real address. An unknown address or a pid mismatch returns zero the caller gets nothing.

This gives each kernel task a clear, auditable boundary around its allocations without requiring separate page directories or hardware context switches. The enforcement is in software so it requires disciplined implementation.

### Tradeoffs

1. Shared kernel address space → No IPC serialisation or context-switch overhead for kernel requests
2. Software-level task isolation via the Ledger Protocol → Lower hardware cost; every allocation is validated against the owning task before use
...
My background is in web and mobile development. JavaScript, React, React Native. I have an engineering degree in Information and Communications Technology, but nothing in my education (only had one course in C) or day job pointed toward operating systems, low-level C, or NASM assembly. I started this project knowing none of it.

The learning process has been books, the OSDev wiki, other people's source code, YouTube, and using AI as a sounding board for concepts that wouldn't click from text alone. Every part of this kernel was written after educating myself on the topic, writing it, failing, and then writing it again. Not just copying a working black box implementation.
