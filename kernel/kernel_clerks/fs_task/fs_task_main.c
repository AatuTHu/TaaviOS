...
request_queue_t *request_queue[MAX_REQ_ENTRIES];
 ...
 *
 */
static void fs_remove_from_queue(request_queue_t *req) {
...
void fs_task_loop() {
    ...
    while (1) {
        ...
        if (request_queue_count > 0 && index != INVALID_IDX) {
            request_queue_t *request = request_queue[index];
            if (request != NULL) {
...
void fs_recovery() {
    ...
    if (current_req_index != -1) {
        request_queue_t *req = request_queue[current_req_index];
