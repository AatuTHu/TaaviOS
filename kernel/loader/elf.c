...
int elf_load(void *data, page_directory_t *page_dir, uint32_t *heap_start_out) {
    ...
    for (uint16_t i = 0; i < header_data->e_phnum; i++) {
        ...
        uint32_t segment_end = phdr->p_vaddr + size;
        if (segment_end > header_end) {
            header_end = segment_end;
        }
    }

    if (heap_start_out) {
        *heap_start_out = header_end;
    }
