...
/*
* Calculate the real sector number in the data region based on the given cluster
*/
...
/*
*  An abstraction for kmalloc really. So many of the helpers and main functions used these lines 
...
uint32_t __fat32_next_cluster(uint32_t cluster) {
    ...
    if(buf == INVALID_BUFFER) return INVALID_CLUSTER;
    ...
    *   we can calcualte the entry index within the previosly read logical block address
    *   Then mask the entry with 0x0FFFFFFF to return only the cluster number
    */
...
int __fat32_read_cluster(uint32_t cluster, uint8_t *buf) {
    ...
    if(sector_number == INVALID_CLUSTER) return STATUS_ERROR;

    /*
    * Loop as many times as there are sector in a cluster 
    * Read the data to given buffer starting from calculated sector number.
    * Every iteration advance the sectors number by the iteration cycle number and the buffer by iteration cycle * (FAT32_SECTOR_SIZE = 512)
    * This turns the cluster's contigous layout into a flat array of data more easily manipulated.
    */
    for(uint8_t i = 0; i < f32_fs.sectors_per_cluster; i++) {
        if(ata_read_sector((sector_number + i), (buf + i * FAT32_SECTOR_SIZE)) == STATUS_ERROR) {
...
uint32_t __fat32_alloc_cluster(void) {
    ...
    // An optimization attempt. By saving the last allocated cluster we can start the allocation from there by calculationg its sector index
    ...
    uint32_t proposed_entry_index = f32_fs.last_allocated_cluster % FAT32_ENTRIES_PER_SECTOR; 
    ...
    */
    for(uint32_t i = starting_sector_index; i < f32_fs.sectors_per_fat; i++) {
        /*
        * Every loop read a sector to allocated buffer. Starting from fat_start and advancing it by i
        * 
        */
        ...
        /*
        * cast the buffer in to a uint32_t pointer array for ease of manipulation
        * fat_entries is an array fat entries
        */
        uint32_t *fat_entries = (uint32_t*)buf;
        
        /*
        * First iteration is the proposed entry index calculated before starting the loop.
        * Every other iteration use 0
        */
        uint32_t real_entry_index = (i == starting_sector_index) ? proposed_entry_index : 0;

        /*
        * Inner loop starting from the real enrty index until j reaches (entries per sector = 512/4)
        * if j is 0 or 1 go to next iteration because the first 2 entries are reserved.
        * by masking the value inside the fat_entries[j] with 0x0FFFFFFF and checking if it then is same as 0x00000000 (free cluster). If it is we have found a free cluster
        */
        for(uint32_t j = real_entry_index; j < FAT32_ENTRIES_PER_SECTOR; j++) {
            if(i == 0 && j < 2) continue;
            if((fat_entries[j] & FAT32_CLUSTER_MASK) == FAT32_CLUSTER_FREE ) {

                // calculate the absolute cluster number across the entire filesystem
                ...
                /*
                * Write to the hard drive starting from fat_start + current i the contents of the buf. Also write it to the second fat table
                * Should writing fail mark the cluster of current J as free cluster and return Invalid cluster.
                */
                ...
                //save index of last allocated cluster so that next allocation can continue from it.
...
int __fat32_write_cluster(uint32_t cluster, const uint8_t *buf) {
    ...
    * while writing to hard drive starting from the sector number the contents of the buffer
    * Every iteration advances sector number by i and buffer by i * (SECTOR_SIZE = 512)
    * This writes the buffers data to contigous physical sectors
    */
    for(int i = 0; i < f32_fs.sectors_per_cluster; i++) {
        if(ata_write_sector(sector_number + i, buf + i * FAT32_SECTOR_SIZE) == -1) {
...
int __fat32_set_cluster(uint32_t cluster, uint32_t value) {
    ...
    *   Then calculate the right index of the entry within read sector by dividing the fat_offset by sizeof uint32_t
    *   Deposit the value to that index.
    */
    ...
    /*
    * Write the change to the actual hard drive. Both the first and the second fat tables.
    */
...
int __fat32_format_83(const char *filename, uint8_t *dst) {
    ...
    */
    ...
    // if there was no dot the filename it was a file or folder name. So no extensions
    if(index_of_dot == -1) {
        for(uint8_t i = 0; i < strlen(filename); i++) {
            ...
            //put them in to dst but change the into upper case. otherwise just deposite
    ...
    //turn the file ext tp upper chase adn deposit to dst. maximum lenght of ext is 3, but can cap before that 
    for(uint8_t i = 0; i < 3 && filename[index_of_dot + i] != '\0'; i++) {
        if (filename[index_of_dot + i] >= 'a' && filename[index_of_dot + i] <= 'z') {
...
int __fat32_search_dir(uint32_t dir_cluster, const uint8_t *name83, uint32_t *out_cluster, uint32_t *out_size, uint8_t  *out_attr) {
    ...
    */
    while(is_done != 1) {
        ...
        * First check that if the directory entrys name is 0x00 (FAT32_DIRENT_FREE). 
        * If it is true set is done to ok. and break because there are no more active entries in the directory.
        * the if the directory entry is 0xE5 (FAT32_DIRENT_DELETED) or its attributes are 0x0F (FAT32_LONG_FILE_NAME) 
        * continue immidialety to next iteration. There is no support for long filenames for now
        */
        for(uint32_t i = 0; i < max_dir_entries ; i++) {
            ...
            //compare the given name to the dir_entrys name. If it a match then we gound the right dir entry
            if(memcmp(dir_entry[i].name, name83, 11) == 0) {

                //Construct a whole cluster from cluster high and cluster low. Sifht the higher cluster by 16 so that the lower goes to shifted part
        ...
        /*
        * If we did not find the filename or did not reach end of the active entries proceed to next cluster
        */
