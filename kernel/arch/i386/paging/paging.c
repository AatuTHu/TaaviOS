...
page_directory_t *paging_create_directory() {
    ...
    page_directory_t *phys_addr = (page_directory_t*)pmm_alloc(); //ask pmm to allocate a physical page dir
    ...
    memcpy(&(*virt_dir)[KERNEL_PD_INDEX_START], &kernel_page_dir[KERNEL_PD_INDEX_START], KERNEL_PD_ENTRIES * sizeof(uint32_t));
...
uint32_t paging_get_phys(page_directory_t *dir, uint32_t virt) {
    ...
    klog("[PAGING]: getting physical location for page directory: %x, with virt: %d\n", (uint32_t)dir, virt);
    uint32_t pd_index = virt >> PD_INDEX_SHIFT;
