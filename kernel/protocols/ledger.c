...
int remove_request() {
    int kill_count = 0;
    DEBUG("[LEDGER][REMOVE]: Dont fear the reaper\n");
    for (int i = 0; i < CLERK_COUNT; i++) {
        for (int j = 0; j < MAX_REQ_ENTRIES; j++) {
            if (request_queue[i][j] != NULL && request_queue[i][j]->status == TERMINATED) {
                DEBUG("[LEDGER][REMOVE]: Reaper came for the request %d, %d\n", i, j);
                kfree(request_queue[i][j]);
                request_queue[i][j] = NULL;
                kill_count++;
            }
        }
    }
    DEBUG("[LEDGER][REMOVE]: Reaper exiting with kill count of %d\n", kill_count);
    return kill_count;
}
...
request_table *fetch_next_task(uint32_t clerk_pid) {
   ...
   DEBUG("[LEDGER][FETCH_NEXT_TASK]: %d came to look for a request!\n", clerk_pid);
